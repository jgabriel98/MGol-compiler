...
void Automata::add_transition(int i, int j, char c) { transitions[{i, c}] = j; }

void Automata::add_final_state(int state) {
	if (state == rej_s) {
		throw new std::logic_error("Um estado final não pode ser o estado de rejeição.");
	}
	final_states.insert(state);
}

bool Automata::is_final_state(int state) { return final_states.count(state); }
...
int Automata::process_string(const std::string &str) {
